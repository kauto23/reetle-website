...
export default function QuestionCard({ question, onAnswer, onNext, nextLabel = 'Next Question', showHint, onDismissHint }: QuestionCardProps) {
  ...
  const handleQuestionMouseEnter = useCallback(() => {
    ...
    hoverTimerRef.current = setTimeout(() => setShowTranslation(true), 2000);
  ...
  const handleSelect = (choiceText: string) => {
    ...
    if (isCorrect && question.masteryHeading) {
  ...
  const revealDelay = (slot: number) => 0.24 + slot * 0.24;
  ...
  return (
    <motion.div
      ...
      exit={{ opacity: 0, y: -16, transition: { duration: 0.8 } }}
    ...
    >
      ...
      <motion.div
        ...
        onMouseEnter={!hasAnswered ? handleQuestionMouseEnter : undefined}
        onMouseLeave={!hasAnswered ? handleQuestionMouseLeave : undefined}
        animate={hasAnswered && !isCorrect ? { x: [0, -8, 8, -6, 6, -3, 3, 0] } : {}}
        transition={hasAnswered && !isCorrect ? { duration: 1.8, ease: 'easeInOut' } : { layout: { duration: 1.2, ease: easeOut } }}
      >
        {showConfetti && <ConfettiBurst />}

        <div className="absolute top-0 left-0 right-0 h-[3px] bg-primary rounded-t-2xl" />

        <p className="text-[20px] sm:text-[22px] font-semibold text-primary leading-relaxed text-center pt-xs relative z-[1]">
          {displayQuestion}
        </p>

        <AnimatePresence>
          {!hasAnswered && question.questionFamiliar && showTranslation && (
            <motion.p
        ...
        {hasAnswered && question.questionCompleteFamiliar && (
          <motion.p
            ...
            animate={{ opacity: 1, y: 0 }}
            transition={{ delay: revealDelay(questionTranslationSlot), duration: 1.6, ease: easeOut }}
          >
      ...
      <AnimatePresence>
        {showHint && !hasAnswered && (
          <motion.div
            className="flex items-center gap-[8px] px-md py-[10px] bg-primary/90 backdrop-blur-sm rounded-b-xl mx-md -mt-[1px]"
      ...
      {/* Answer choices */}
      <motion.div layout className="flex flex-col gap-[8px] mt-sm" transition={{ layout: { duration: 1.2, ease: easeOut } }}>
                  ...
                  <AnimatePresence>
                    {hasAnswered && choice.textFamiliar && (
                      <motion.p
                        ...
                        initial={{ opacity: 0, y: 14 }}
                        animate={{ opacity: 1, y: 0 }}
                        transition={{ delay: revealDelay(index), duration: 1.6, ease: easeOut }}
                      >
                        {choice.textFamiliar}
                      </motion.p>
                    )}
      ...
      <AnimatePresence>
        {hasAnswered && (
          <motion.div
            ...
            initial={{ opacity: 0, y: 14 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ delay: revealDelay(feedbackSlot), duration: 1.6, ease: easeOut, layout: { duration: 1.2, ease: easeOut } }}
          >
            <motion.div
              className={`p-md rounded-xl ${isCorrect ? 'bg-correct-bg border border-correct/20' : 'bg-incorrect-bg border border-incorrect/20'}`}
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              transition={{ delay: revealDelay(feedbackSlot), duration: 1.6, ease: easeOut }}
            >
            ...
            {isCorrect && question.masteryHeading && (
              <motion.div
                ...
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                transition={{ delay: revealDelay(masterySlot), duration: 1.6, ease: easeOut }}
              >
                <div className="flex items-center gap-sm relative z-[1]">
                  <motion.span
                    className="text-[28px] shrink-0"
                    animate={{ rotate: [0, -12, 12, -8, 0], scale: [1, 1.25, 1] }}
                    transition={{ delay: revealDelay(masterySlot) + 1.2, duration: 2.4 }}
                  >
                    🎉
                  </motion.span>
                  <div>
                    <p className="text-[15px] font-bold text-primary">{question.masteryHeading}</p>
                    {question.masteryText && (
                      <p className="text-[13px] text-text-secondary mt-[2px]">{question.masteryText}</p>
                    )}
                  </div>
                </div>
              </motion.div>
            ...
            {onNext && (
              <motion.button
                ...
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                transition={{ delay: revealDelay(buttonSlot), duration: 1.6, ease: easeOut }}
